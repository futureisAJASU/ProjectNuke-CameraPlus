...
internal fun processClassicYuvFusionJob(
    ...
    try {
        ...
        markStage("YUV_ALIGNING", "YUV ?ÑÎ†à?ÑÏùÑ ?ïÎ†¨?òÎäî Ï§ëÏûÖ?àÎã§.")
        cancellation.throwIfCancelled()
        ...
        frames.forEachIndexed { index, frame ->
            cancellation.throwIfCancelled()
            onStatus("YUV ?ÑÎ†à?ÑÏùÑ ?ïÎ†¨?òÎäî Ï§ëÏûÖ?àÎã§.")
            if (frame === reference) {
        ...
        markStage("YUV_MERGING", "YUV ?ÑÎ†à?ÑÏùÑ ?©ÏÑ±?òÎäî Ï§ëÏûÖ?àÎã§.")
        cancellation.throwIfCancelled()
        ...
        markStage("YUV_DENOISE_SHARPEN", "?∏Ïù¥Ï¶àÏ? ?†Î™Ö?ÑÎ? Î≥¥Ï†ï?òÎäî Ï§ëÏûÖ?àÎã§.")
        cancellation.throwIfCancelled()
        finalBitmap = finishClassicFusion(merged, params, cancellation)
        cancellation.throwIfCancelled()
        val lookDoneAt = System.currentTimeMillis()
        markStage("YUV_EXPORTING", "Í≤∞Í≥ºÎ•??Ä?•Ìïò??Ï§ëÏûÖ?àÎã§.")
        ...
        saveJobJson(jobFile.parentFile ?: error("Job directory missing"), job)
        cancellation.throwIfCancelled()
        onStatus("Ï≤òÎ¶¨Í∞Ä ?ÑÎ£å?òÏóà?µÎãà??")
        return finalFile
    } catch (oom: OutOfMemoryError) {
