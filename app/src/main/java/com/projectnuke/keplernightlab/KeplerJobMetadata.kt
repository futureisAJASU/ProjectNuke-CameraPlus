...
internal class ProcessingCleanupRequiredException : IllegalStateException(
    "?´ì „ ì²˜ë¦¬ ?‘ì—…???Œì¼ ?•ë¦¬ê°€ ?„ë£Œ?˜ì? ?Šì•„ ì§€ê¸ˆì? ?¤ì‹œ ?©ì„±?????†ìŠµ?ˆë‹¤."
)
...
internal class JobRecoveryMutationBlockedException(
    ...
    when (outcome) {
        JobRecoveryMutationGateOutcome.BLOCKED_DEAD_OPERATION -> "?´ì „ ?¤í–‰???‘ì—… ?Œìœ ê¶Œì´ ?„ì§ ë³µêµ¬?˜ì? ?Šì•„ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_HANDOFF -> "ì´¬ì˜ ê²°ê³¼??ì²˜ë¦¬ ?¸ê³„ê°€ ?„ì§ ?„ë£Œ?˜ì? ?Šì•„ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_ORPHANED_JOB_METADATA -> "?‘ì—… ë©”í??°ì´?°ê? ?†ì–´ ë³µêµ¬ ?•ì¸ ?„ì—???‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_PROCESSING_CLEANUP -> "?´ì „ ì²˜ë¦¬ ?‘ì—…???Œì¼ ?•ë¦¬ê°€ ?„ë£Œ?˜ì? ?Šì•„ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_AMBIGUOUS_RECOVERY -> "ë³µêµ¬?˜ì? ?Šì? ?‘ì—… ì¦ê±°ê°€ ?ˆì–´ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_PUBLIC_COMMIT_MISSING -> "ê³µê°œ ê²°ê³¼??ì»¤ë°‹ ì¦ê±°ê°€ ?†ì–´ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_EXPORT_VERIFICATION -> "ê³µê°œ ê²°ê³¼ë¥??•ì¸?˜ì? ëª»í•´ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_EXPORT_SETTLEMENT -> "ê³µê°œ ?´ë³´?´ê¸°???‘ì—… ?•ë¦¬ ?•ì¸???ë‚˜ì§€ ?Šì•„ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_INVALID_PROCESSING_JOURNAL -> "ì²˜ë¦¬ ë³µêµ¬ ê¸°ë¡???½ì„ ???†ì–´ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_REPROCESS_QUARANTINE -> "ë³µêµ¬ ì¤‘ì¸ ?‘ì—…?€ ì§€ê¸?ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.INSPECTION_FAILED -> "?‘ì—… ë³µêµ¬ ?íƒœë¥??•ì¸?˜ì? ëª»í•´ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_INVALID_EXPORT_JOURNAL -> "?´ë³´?´ê¸° ë³µêµ¬ ê¸°ë¡???½ì„ ???†ì–´ ì§€ê¸ˆì? ?‘ì—…??ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
        JobRecoveryMutationGateOutcome.BLOCKED_SETTLED_JOURNAL -> "?„ë£Œ??ì²˜ë¦¬ ê¸°ë¡???•ë¦¬ê°€ ?ë‚˜ì§€ ?Šì•„ ì§€ê¸ˆì? ê²°ê³¼ ê²½ë¡œë¥?ë³€ê²½í•  ???†ìŠµ?ˆë‹¤."
    ...
    internal fun recordProcessingCleanupRequired(
        ...
        update(jobDir) { job ->
            ...
            job.put("recoveryState", PROCESSING_CLEANUP_REQUIRED)
                ...
                .put("lastRecoveryMessage", "ì²˜ë¦¬ ê²°ê³¼??ë³´ì¡´?˜ì—ˆì§€ë§??´ì „ ?‘ì—…???Œì¼ ?•ë¦¬ê°€ ?„ì§ ?„ë£Œ?˜ì? ?Šì•˜?µë‹ˆ??")
                .put("recoveredAt", System.currentTimeMillis())
                .put("recoveryMessage", "?´ì „ ì²˜ë¦¬ ?‘ì—…???Œì¼ ?•ë¦¬ê°€ ?„ë£Œ?˜ì? ?Šì•„ ì§€ê¸ˆì? ?¤ì‹œ ?©ì„±?????†ìŠµ?ˆë‹¤.")
            if (operationId != null) {
            ...
            if (committedUnverified) {
                job.put("recoveryState", "PUBLIC_EXPORT_COMMITTED_PENDING_VERIFICATION")
                    ...
                    .put("lastRecoveryMessage",
                        "?±ì´ ?¤ì‹œ ?œìž‘?????„ë£Œ???´ë³´?´ê¸°???•ì¸?˜ì? ?Šì•„ ì¶”ê? ?•ì¸???„ìš”?©ë‹ˆ??")
                    .put("recoveryMessage",
                        "ê³µê°œ ?´ë³´?´ê¸° ê²°ê³¼???•ì¸???„ë£Œ?˜ì? ?Šì•„ ì¶”ê? ?•ì¸???„ìš”?©ë‹ˆ??")
            } else {
                job.put("recoveryState", "STABLE")
                    .put("lastRecoveryClassification", "RECOVERED")
                    .put("lastRecoveryMessage", "?±ì´ ?¤ì‹œ ?œìž‘?????„ë£Œ???´ë³´?´ê¸° ê²°ê³¼ë¥??•ì¸?ˆìŠµ?ˆë‹¤.")
                job.remove("recoveryMessage")
            }
    ...
    internal fun finalizeRecoveredProcessingHandoff(
        ...
        update(jobDir) { job ->
            ...
            job.put("recoveryState", "STABLE")
                ...
                .put("lastRecoveryMessage", "ì²˜ë¦¬ê°€ ?œìž‘?˜ê¸° ???´ì „ ?¤í–‰??ì¢…ë£Œ?˜ì—ˆì§€ë§??ë³¸ ?„ë ˆ?„ì„ ?¤ì‹œ ?¬ìš©?????ˆìŠµ?ˆë‹¤.")
                .put("recoveredAt", System.currentTimeMillis())
            job.remove("recoveryMessage")
            job.remove(PROCESSING_HANDOFF_RUNTIME_SESSION_ID)
            job.remove(PROCESSING_HANDOFF_OPERATION_ID)
